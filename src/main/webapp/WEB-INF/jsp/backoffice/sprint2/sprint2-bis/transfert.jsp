            ...
            <section class="panel">
                <div class="panel-header">
                    <div>
                        ...
                        <h2>Creer un transfert de visa</h2>
                    </div>
                </div>

                <p class="overview-text">
                    Le transfert cree un dossier a scanner vers un nouveau passeport. La carte de resident reste celle du dossier parent accepte.
                </p>

                <form id="s2-trans-form" class="admin-form">
                    ...
                    <div style="display:flex;gap:10px;margin-top:18px;">
                        <button class="button primary important-blue" type="submit">Creer le transfert</button>
                        <a class="button secondary" href="${pageContext.request.contextPath}/backoffice">Annuler</a>
                    </div>
                </form>
                <div id="s2-trans-result" class="result-card hidden"></div>
            </section>
        </main>
    </div>
</div>
...
document.getElementById('s2-trans-form').addEventListener('submit', async function (ev) {
    ev.preventDefault();
    const f = ev.target;
    ...
    [
        ...
        ['passeport.numero', 'Numero du nouveau passeport'],
        ['passeport.dateDelivrance', 'Date de delivrance du nouveau passeport'],
        ['passeport.dateExpiration', 'Date d expiration du nouveau passeport']
    ].forEach(([name, label]) => {
        if (!get(name)) erreurs.push(`Le champ ${label} est obligatoire`);
    });

    const dateDelivrance = get('passeport.dateDelivrance');
    const dateExpiration = get('passeport.dateExpiration');
    if (dateDelivrance && dateExpiration && new Date(dateExpiration) < new Date(dateDelivrance)) {
        erreurs.push("La date d'expiration du passeport doit etre posterieure a la date de delivrance");
    }

    if (erreurs.length > 0) {
        const resultEl = document.getElementById('s2-trans-result');
        if (resultEl) {
    ...
    const payload = {
        ...
        passeport: {
            numero: get('passeport.numero'),
            dateDelivrance: get('passeport.dateDelivrance'),
            dateExpiration: get('passeport.dateExpiration')
        },
        resident: get('resident') === 'true',
        demandeParenteId: parentId
    };

    const r = await fetch(`${contextPath}/demandes/${parentId}/transfert-visa`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload)
    });
    const j = await r.json();
    const result = document.getElementById('s2-trans-result');

    if (r.ok && j.success) {
        result.classList.remove('hidden');
        result.style.borderColor = 'var(--success)';
        result.innerHTML = '<strong>Transfert cree</strong><span>Redirection vers le scan du dossier...</span>';
        setTimeout(() => {
